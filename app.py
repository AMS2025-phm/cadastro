...
@app.before_request
def verificar_login():
    # Permite acesso às rotas de login e registro sem estar logado
    if not session.get('user_id') and (request.endpoint is None or (request.endpoint not in ['login', 'register', 'static'] and not request.endpoint.startswith('static'))):
        return redirect(url_for('login'))
