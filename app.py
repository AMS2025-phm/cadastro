...
def exportar_excel_e_enviar_email():
    ...
    for medida in info.get("medidas", []):
        if isinstance(medida, list) and len(medida) == 4:
            ...
            if tipo in abas:
                ...
                abas[tipo].
                append([unidade, comp, larg, round(area, 2)])
        else:
    ...
    # CORREÇÃO: Nome do arquivo Excel agora é apenas o nome da unidade com extensão .xls
    # Sanitização de caracteres especiais
    nome_arquivo = f"{unidade}.xls".replace(" ", "_").replace("/", "_").replace("\\", "_").replace(":", "_").replace("*", "_").replace("?", "_").replace("\"", "_").replace("<", "_").replace(">", "_").replace("|", "_")
