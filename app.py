...
def exportar_excel_e_enviar_email():
    ...
    # Removido "Localidade" do cabeçalho das abas de medidas
    for ws in abas.values():
        ws.append(["Unidade", "Comprimento (m)", "Largura (m)", "Área (m²)"])

    for medida in info.get("medidas", []):
        if isinstance(medida, list) and len(medida) == 4:
            tipo, comp, larg, area = medida
            if tipo in abas:
                # Removido 'local' dos dados adicionados às abas de medidas
                abas[tipo].append([unidade, comp, larg, round(area, 2)])
        else:
    ...
    # CORREÇÃO: Nome do arquivo Excel agora é apenas o nome da unidade com extensão .xlsx
    # Sanitização de caracteres especiais
    nome_arquivo = f"{unidade}.xlsx".replace(" ", "_").replace("/", "_").replace("\\", "_").replace(":", "_").replace("*", "_").replace("?", "_").replace("\"", "_").replace("<", "_").replace(">", "_").replace("|", "_")
